#include "lib.h"
//XXX No concurrency, need to add that !!
#define MAX_OBJS 16
struct object{
     long id;
     long size;
...
{
  //XXX what is objs pointing to??
   struct object *obj = objs + objid - 2;
   if(objid < 2)
       return -1;
   if(obj->id != objid)
       return -1;
     //XXX need to implement read into the buffer.
...
{
  //XXX st_ino is the inode number acc. to man stat.
  //what is obj pointing to ?? How can we use the inode number to get that object.
...
int objstore_init(struct objfs_state *objfs)
{
   ...
   // XXX read requires pointer to buff as the third parameter, how can we write (char*)objs ??
   //why are we checking read ??s
   ...
   //XXX what does objfs->objstore_data exactly contain ??
   //How can we simply make it point to objs ? 
...
int objstore_destroy(struct objfs_state *objfs)
{
   ...
   //XXX why are we checking write on destroying?
